...
def index():
    ...
    if 'user_id' not in session:
        ...
        oauth_url = (
            ...
            f"appid={CORP_ID}&redirect_uri={quote(OAUTH2_CALLBACK, safe='')}&response_type=code"
            f"&scope={OAUTH2_SCOPE}&agentid={AGENT_ID}&state={state}#wechat_redirect"
        )
        return f'''
        <!DOCTYPE html>
        <html>
...
def oauth_callback():
    ...
    try:
        ...
        if resp['errcode'] == 0:
            # 获取用户信息
            user_id = resp.get('UserId') or resp.get('userid')  # getuserinfo返回小写userid，兼容大小写
